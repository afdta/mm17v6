    ...
    (function(){
      try{
        ...
        boxtext.enter().append("p").classed("data-text",true).merge(boxtext).text(function(d,i){
          if(i==0){
            return format.possessive(metname(cbsa).nameshort) + " overall rank: " + (d==null ? "N/A" : format.rank(d));
          }
          else{
            return d.variable + ": " + (d.val==null ? "N/A" : format.pct1(d.val)) + " (" + (d.rank==null ? "N/A" : format.rank(d.rank)) + ")"
          }
        })
...
function inclusion_detail(container, url, state){
	...
	dom.wrap = d3.select(container)
		...
		;
	dom.titlebox = dom.wrap.append("div").style("margin","0em 0em 1em 0em");
	...
	function init(){
		...
		var mains2 = mains_enter.merge(mains).style("margin","4em 0em 0em 0em");
	...
	function drawIncDetail(data, extent_object, cbsa, groups, wrap){
		...
		var valScale = function(v){
			if(v < extent[0]){
				return {x:(val_scale(extent[0])-6)+"%", outlier:true}
			}
			else if (v > extent[1]){
				return {x:(val_scale(extent[1])+6)+"%", outlier:true}
			}
			else{
				return {x:val_scale(v)+"%", outlier:false}
			}
		};
		...
		var tickValues = val_scale.ticks(5).map(function(d){
		...
		var v_lines = groups.select("g.v_gridline_layer").selectAll("line").data(function(d,i){
			...
			v_lines.exit().remove();
			v_lines.enter().append("line").merge(v_lines).attr("stroke","#aaaaaa")
											.attr("stroke-width","1")
											.attr("stroke-dasharray","2,3")
											.style("shape-rendering","crispEdges")
		...
		var avgs = avg_dots.enter().append("circle")
			...
			var svgs_u = groups.select("g.anno_layer").selectAll("svg").data(function(d,i){
				...
				var m = d.vals.map(function(d,i){
					...
					r.x = metro !== null ? val_scale(metro.Value) : 100;
...
function main(){
	...
	var menu_tiles = iselect_wrap
			.append("div")
			.classed("c-fix",true)
			.selectAll("div.ranking-tile").data([{id:"growth", label:"Growth"},{id:"prosperity", label:"Prosperity"},{id: "inclusion", label:"Inclusion"}]).enter().append("div")
			.style("float","left")
			.style("width","100px")
			.style("height","80px")
	...
	var menu_tile_labels = menu_tiles.append("p").text(function(d,i){return d.label})
			  .style("font-size","0.8em")
			  .style("margin","0.5em")
			  .style("line-height","1em")
			  .style("text-align","center");

	var menu_tile_ranks = menu_tiles.append("p").text(function(d,i){return d.label})
